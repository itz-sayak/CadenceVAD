#!/usr/bin/env python3
"""Benchmark CadenceVAD ONNX execution providers on a Colab GPU runtime.
...
def _download_model(commit: str, destination: Path) -> None:
    url = (
        "https://raw.githubusercontent.com/itz-sayak/CadenceVAD/"
        f"{commit}/models/cadencevad-v0.1/cadencevad-stream.onnx"
    )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cadencevad-colab-") as directory:
        model = Path(directory) / "cadencevad-stream.onnx"
        ...
        report = {
            "schema": "cadencevad-colab-provider-benchmark-v1",
        ...
        print("CADENCEVAD_COLAB_BENCHMARK=" + json.dumps(report, sort_keys=True))
