#!/usr/bin/env python3
"""Run a leakage-safe multi-seed CadenceVAD training sweep on one development split."""
...
from cadencevad.checkpoint import load_checkpoint_data
from cadencevad.config import ProjectConfig
from cadencevad.manifest import validate_manifest_group_separation
from cadencevad.train import select_training_device, train
...
def _source_tree_sha256(repository: Path) -> str:
    files = sorted((repository / "src" / "cadencevad").rglob("*.py"))
...
def main() -> None:
    ...
    report: dict[str, Any] = {
        "schema": "cadencevad-training-sweep-v1",
        "name": sweep["name"],
