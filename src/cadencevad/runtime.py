...
def _validate_tensor_contract(
    ...
    # A model using the causal running-norm input stage threads one extra state
    # tensor holding the per-feature running mean and variance.
    if str(model.get("input_norm", "layer")) == "ema":
        running_shape = (None, 2 * feature_dim)
        expected_inputs["running_norm"] = running_shape
        expected_outputs["next_running_norm"] = running_shape
