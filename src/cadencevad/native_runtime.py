...
class MacosNativeVadModel:
    """Process-level owner for an embedded Apple Accelerate CadenceVAD library."""
    ...
    def __init__(
        ...
        self.library.cadencevad_state_size.argtypes = []
        self.library.cadencevad_state_size.restype = ctypes.c_size_t
        self.library.cadencevad_init.argtypes = [ctypes.c_void_p]
        self.library.cadencevad_init.restype = ctypes.c_int
        self.library.cadencevad_reset.argtypes = [ctypes.c_void_p]
        self.library.cadencevad_destroy.argtypes = [ctypes.c_void_p]
        self.library.cadencevad_push.argtypes = [
            ctypes.c_void_p,
        ...
        self.library.cadencevad_push.restype = ctypes.c_size_t
        self.state_size = int(self.library.cadencevad_state_size())
        if self.state_size <= 0:
            raise RuntimeError("CadenceVAD native library returned an invalid state size")
...
class MacosNativeVadStream:
    def __init__(
        ...
        self._closed = False
        if owner.library.cadencevad_init(self._state) != 0:
            raise RuntimeError("CadenceVAD native state initialization failed")

    def reset(self) -> None:
        self._ensure_open()
        self.owner.library.cadencevad_reset(self._state)
        self.detector.reset()

    def close(self) -> None:
        if not self._closed:
            self.owner.library.cadencevad_destroy(self._state)
            self._closed = True

    def _ensure_open(self) -> None:
        if self._closed:
            raise RuntimeError("CadenceVAD native stream is closed")
    ...
    def push(self, audio: object) -> tuple[np.ndarray, list[VadEvent]]:
        ...
        emitted = int(
            self.owner.library.cadencevad_push(
                self._state,
                sample_pointer,
                samples.size,
                output_pointer,
        ...
        if emitted == _INSUFFICIENT_OUTPUT:
            raise RuntimeError("CadenceVAD native output capacity calculation failed")
...
def _bundled_native_source_dir() -> Path:
    ...
    if repository.is_dir():
        return repository
    raise RuntimeError("CadenceVAD macOS native sources are missing from this installation")
...
def _native_source_digest(source: Path) -> str:
    ...
    for name in (
        "cadencevad_native.c",
        "cadencevad_native.h",
        "cadencevad_weights.c",
        "cadencevad_weights.h",
    ):
        path = source / name
        if not path.is_file():
            raise RuntimeError(f"CadenceVAD native source is incomplete: missing {name}")
...
def _bundled_native_library() -> Path:
    ...
    if compiler is None:
        raise RuntimeError("clang is required to build CadenceVAD's bundled runtime")
    ...
    configured_cache = os.environ.get("CADENCEVAD_CACHE_DIR")
    cache_root = (
        Path(configured_cache).expanduser()
        if configured_cache
        else Path.home() / "Library" / "Caches" / "cadencevad"
    )
    output_dir = cache_root / "native" / revision
    library = output_dir / "libcadencevad_native.dylib"
    if library.is_file():
        return library
        ...
        try:
            subprocess.run(
                [
                    ...
                    f"-I{source}",
                    str(source / "cadencevad_native.c"),
                    str(source / "cadencevad_weights.c"),
                    "-framework",
                    "Accelerate",
                    "-dynamiclib",
                    "-o",
            ...
            raise RuntimeError(f"CadenceVAD native build failed: {message}") from exc
        finally:
