...
def train(
    ...
    model = CadenceVad(config.model).to(device)
    ...
    # Each item decodes a whole source file, so the workers are the bottleneck on a
    # fast GPU. Keeping them alive across epochs avoids paying process startup and
    # dataset re-indexing every epoch.
    loader_options: dict[str, object] = {"num_workers": config.training.num_workers}
    ...
    averaging = config.training.weight_averaging == "ema"
    ...
    for epoch in range(1, config.training.epochs + 1):
        ...
        for audio, labels, auxiliary_labels in train_loader:
            ...
            if config.training.ranking_weight > 0.0:
                loss = loss + config.training.ranking_weight * pairwise_ranking_loss(
                    logits,
                    labels,
                    margin=config.training.ranking_margin,
                    pairs=config.training.ranking_pairs,
                )
            ...
            optimizer.step()
            if averaging:
                decay = config.training.weight_averaging_decay
                with torch.no_grad():
                    for name, value in model.state_dict().items():
                        if value.dtype.is_floating_point:
                            averaged[name].mul_(decay).add_(value.float(), alpha=1.0 - decay)
                        else:
        ...
        if not averaging and metrics["f1"] > best_f1:
            best_f1 = metrics["f1"]
            torch.save(
                {
    ...
    if averaging:
        # The averaged weights are the released model: no epoch is "selected", so
        # the run is reproducible and free of checkpoint-selection variance.
        model.load_state_dict({name: value for name, value in averaged.items()})
        metrics, threshold = evaluate(frontend, model, valid_loader, device)
        torch.save(
            {
                "model": model.state_dict(),
                ...
                "metrics": metrics,
                "selection": "polyak-averaged weights",
            },
            best_path,
        )
