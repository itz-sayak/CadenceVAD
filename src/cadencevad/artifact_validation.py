...
RELEASE_MANIFEST_VERSION = "cadencevad-v0.1"
_EXPECTED = {
    "cadencevad-stream.onnx": "9a88e34bf3118d60e25a16cb622cb394e2f3ab71445b0aa5957df6f1d5f1b6ba",
    "cadencevad-stream.json": "5052faab08f4a50c9b628b7a57e1747d368b3e7d4cedd3c92da8e3095ba3ea65",
}
...
# Fields added after the v0.1 release. A sidecar may carry them, but anything
# outside the union of required and optional keys is still rejected, so an
# unrecognised or tampered field cannot ride along unnoticed.
_FEATURE_OPTIONAL_KEYS = {"mel_mean_subtraction"}
_MODEL_OPTIONAL_KEYS = {"input_norm", "input_norm_rate", "input_norm_variance_floor"}
_DETECTOR_OPTIONAL_KEYS: set[str] = set()
...
def validate_sidecar_metadata(path: str | Path) -> None:
    """Validate a CadenceVAD release sidecar without trusting its model digest."""
    path = Path(path)
    try:
    ...
    _exact_keys(feature, _FEATURE_KEYS, "feature", _FEATURE_OPTIONAL_KEYS)
    _exact_keys(model, _MODEL_KEYS, "model", _MODEL_OPTIONAL_KEYS)
    _exact_keys(detector, _DETECTOR_KEYS, "detector", _DETECTOR_OPTIONAL_KEYS)
