...
def _portable_source_dir() -> Path:
    """Backend-neutral DSP layer shared by the Accelerate and portable runtimes."""
    packaged = Path(__file__).resolve().parent / "_native" / "portable"
    if packaged.is_dir():
        return packaged
    repository = Path(__file__).resolve().parents[2] / "native" / "portable"
    if repository.is_dir():
        return repository
    raise RuntimeError("CadenceVAD portable DSP sources are missing from this installation")
 ...
 * and is not automatically covered by the CadenceVAD repository source licence.
 */
#ifndef CADENCEVAD_WEIGHTS_H
#define CADENCEVAD_WEIGHTS_H
...
#endif
"""
    (output / "cadencevad_weights.h").write_text(header, encoding="utf-8")
    ...
    (output / "cadencevad_weights.c").write_text(source, encoding="utf-8")

    metadata = {
        "format": "cadencevad-accelerate-v1",
    ...
    (output / "cadencevad_native.json").write_text(
...
def build_macos_native(
    ...
    common = [
        ...
        f"-I{portable}",
        f"-I{weights}",
        str(native / "cadencevad_native.c"),
        str(portable / "fv_dsp_accelerate.c"),
        str(weights / "cadencevad_weights.c"),
        "-framework",
        "Accelerate",
    ]
    library = output / "libcadencevad_native.dylib"
    subprocess.run(
        [*common, "-dynamiclib", "-o", str(library)],
        check=True,
    )
    benchmark = output / "cadencevad_benchmark"
    subprocess.run(
...
def benchmark_macos_native(
    ...
    if compiler is None:
        raise RuntimeError("clang is required")
    result = {
        "schema": "cadencevad-native-benchmark-v1",
