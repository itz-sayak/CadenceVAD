...
try:
    ...
    raise ImportError(
        "Pipecat support requires `pip install 'cadencevad[pipecat]'`"
    ) from exc
...
class CadenceVadPipecatAnalyzer(VADAnalyzer):
    ...
    def __init__(
        ...
        *,
        ...
        if sample_rate is not None and sample_rate not in (8_000, 16_000):
            raise ValueError("CadenceVAD Pipecat adapter supports 8 kHz or 16 kHz mono PCM")
    ...
    def voice_confidence(self, buffer: bytes) -> float:
        ...
        if probabilities.size != 1:
            raise RuntimeError("CadenceVAD Pipecat adapter expected one 10 ms frame")
...
__all__ = ["CadenceVadPipecatAnalyzer"]
