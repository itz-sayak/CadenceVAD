...
try:
    ...
    raise ImportError(
        "LiveKit support requires `pip install 'cadencevad[livekit]'`"
    ) from exc
...
class CadenceVadLiveKit(agents.vad.VAD):
    ...
    @property
    def model(self) -> str:
        return "cadencevad-v0.1"

    @property
    def provider(self) -> str:
        if isinstance(self._owner, MacosNativeVadModel):
            return "CadenceVAD Accelerate"
        return "CadenceVAD ONNX"

    def stream(self) -> CadenceVadLiveKitStream:
        return CadenceVadLiveKitStream(
            self,
...
class CadenceVadLiveKitStream(agents.vad.VADStream):
    _SAMPLE_RATE = 16_000
    _HOP_SAMPLES = 160

    def __init__(
        self,
        vad: CadenceVadLiveKit,
        stream: VadStream,
        *,
    ...
    def _process_hop(self, frame: rtc.AudioFrame) -> None:
        ...
        if probabilities.size != 1:
            raise RuntimeError("CadenceVAD LiveKit adapter expected one probability per 10 ms hop")
    ...
    async def _main_task(self) -> None:
        ...
        async for item in self._input_ch:
            ...
            if item.num_channels != 1:
                raise ValueError("CadenceVAD LiveKit adapter accepts mono audio only")
...
__all__ = ["CadenceVadLiveKit", "CadenceVadLiveKitStream"]
