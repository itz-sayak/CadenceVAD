...
test("Astro renders the complete static benchmark report", async () => {
  ...
  assert.match(
    html,
    /<title>CadenceVAD: Fast Voice Activity Detection for Voice Calls<\/title>/i,
  );
  assert.match(
    html,
  ...
  assert.match(html, /"name":"Sayak Dutta"/);
  ...
  assert.match(html, /9\.02 microseconds/);
  ...
  assert.match(html, /Only CadenceVAD executes in this browser demo/);
...
test("keeps claims, methodology, and Astro islands explicit in source", async () => {
  ...
  assert.match(page, /competitor RTF divided by CadenceVAD native RTF/);
  assert.match(page, /39\.67 h of AVA-Speech/);
  assert.match(page, /0\.886 \(ResectNet\)/);
  assert.match(page, /35\.8% to 15\.7%/);
  ...
  assert.match(page, /seed variance is sd 0\.0127/);
  assert.match(page, /0\.902 \/ 0\.897/);
  ...
  assert.match(
    layout,
    /CadenceVAD: Fast Voice Activity Detection for Voice Calls/,
  );
  ...
  assert.match(wranglerConfig, /"name": "cadencevad-report"/);
