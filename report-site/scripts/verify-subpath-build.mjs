...
for (const asset of [
  ...
  assert.ok(
    rendered.includes(`/cadencevad/${asset}`),
    `subpath build does not reference /cadencevad/${asset}`,
  );
}

assert.match(rendered, /["'`]\/cadencevad\/["'`]/);
assert.match(rendered, /models\/cadencevad-stream\.onnx/);
assert.match(rendered, /vad-audio-processor\.js/);
assert.doesNotMatch(rendered, /\/cadencevad(?:benchmarks|favicon|models|NOTICE|vad-)/);
console.log("verified /cadencevad static asset URLs");
