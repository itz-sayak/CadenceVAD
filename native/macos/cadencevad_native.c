...
static void affine(
    ...
    fv_sgemv(rows, columns, 1.0f, weight, input, 1.0f, output);
}
...
int cadencevad_init(CadenceVadState *state) {
    ...
    state->dft_setup = (void *)fv_rfft_create(FV_N_FFT);
...
void cadencevad_destroy(CadenceVadState *state) {
    ...
    if (state->dft_setup != NULL) {
        fv_rfft_destroy((FvRealFft *)state->dft_setup);
    }
    memset(state, 0, sizeof(*state));
}
...
int cadencevad_extract_features(
    ...
    fv_rfft_execute(
        (const FvRealFft *)state->dft_setup,
        state->fft_even,
        state->fft_odd,
        state->fft_real,
        state->fft_imag
    ...
    /* The packed real DFT contract carries a factor of two. */
...
float cadencevad_process_hop(
    ...
    if (cadencevad_extract_features(state, samples, state->feature) != 0) {
        return NAN;
    }
    return cadencevad_model_step(state, state->feature);
}
...
size_t cadencevad_push(
    ...
    if (sample_count > SIZE_MAX - state->pending_count) {
        return CADENCEVAD_INSUFFICIENT_OUTPUT;
    }
    ...
    while (consumed < sample_count) {
        ...
        if (state->pending_count == FV_HOP_SAMPLES) {
            probabilities[emitted++] =
                cadencevad_process_hop(state, state->pending);
