...
CadenceVAD should be instantiated once per process and keep only the small
...
and a chunk-invariant causal resampler in `cadencevad.telephony`.

```python
from cadencevad.config import DetectorConfig
...
vad = CadenceVadLiveKit.load(threads=1)
```

`CadenceVadLiveKit` implements LiveKit Agents' `VAD` and `VADStream` contracts.
...
Create the `CadenceVadLiveKit` owner once per worker. Calling `stream()` creates
...
vad_analyzer = CadenceVadPipecatAnalyzer()
```
...
```python
native_vad_analyzer = CadenceVadPipecatAnalyzer.load_native()
```
...
| CadenceVAD | Local macOS/web/PSTN research and controllable training | Current India/GCC accuracy gate is still open |
...
`benchmarks/cadencevad-v0.1/external-runtime-m4-pro.json` reports 3.53× lower
