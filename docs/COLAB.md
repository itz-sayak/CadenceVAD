...
python scripts/run_training_sweep.py \
  ...
  --output /content/drive/MyDrive/cadencevad/india-gcc
```
...
CadenceVAD is a small, stateful model. A T4 can be slower than a CPU for one call because every
...
The recorded Tesla T4 run in `benchmarks/cadencevad-v0.1/onnx-provider-colab-t4.json` used the
