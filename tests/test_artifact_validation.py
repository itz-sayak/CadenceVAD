...
from cadencevad.artifact_validation import (
    validate_bundled_artifacts,
    validate_sidecar_metadata,
)

ROOT = Path(__file__).parents[1] / "models" / "cadencevad-v0.1"


def _copy_release(tmp_path: Path) -> Path:
    model = tmp_path / "cadencevad-stream.onnx"
    shutil.copy2(ROOT / model.name, model)
    shutil.copy2(ROOT / "cadencevad-stream.json", model.with_suffix(".json"))
    return model


def test_bundled_release_artifacts_validate() -> None:
    assert validate_bundled_artifacts(ROOT / "cadencevad-stream.onnx") == (
        ROOT / "cadencevad-stream.onnx"
    ).resolve()
...
def test_sidecar_schema_rejects_invalid_values_without_digest_bypass(tmp_path: Path) -> None:
    sidecar = tmp_path / "cadencevad-stream.json"
