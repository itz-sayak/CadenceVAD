...
from cadencevad.native_export import _native_source_dir, build_macos_native, export_macos_native
from cadencevad.native_runtime import MacosNativeVadModel


def _test_checkpoint() -> Path:
    return Path(
        os.environ.get(
            "CADENCEVAD_TEST_CHECKPOINT",
            "models/cadencevad-v0.1/cadencevad-v0.1.pt",
        )
    )
...
def test_native_sources_are_available() -> None:
    ...
    assert (source / "cadencevad_native.c").is_file()
    assert (source / "cadencevad_native.h").is_file()
    assert (source / "benchmark.c").is_file()
    assert (source / "cadencevad_weights.c").is_file()
    assert (source / "cadencevad_weights.h").is_file()
...
def test_native_runtime_matches_pytorch(tmp_path: Path) -> None:
    ...
    library.cadencevad_state_size.argtypes = []
    ...
    library.cadencevad_reset.argtypes = [ctypes.c_void_p]
    library.cadencevad_extract_features.argtypes = [
        ctypes.c_void_p,
        ctypes.POINTER(ctypes.c_float),
        ctypes.POINTER(ctypes.c_float),
    ]
    library.cadencevad_model_step.argtypes = [
        ctypes.c_void_p,
        ctypes.POINTER(ctypes.c_float),
    ]
    library.cadencevad_model_step.restype = ctypes.c_float
    library.cadencevad_push.argtypes = [
        ctypes.c_void_p,
        ctypes.POINTER(ctypes.c_float),
        ctypes.c_size_t,
        ctypes.POINTER(ctypes.c_float),
    ...
    library.cadencevad_push.restype = ctypes.c_size_t

    state = ctypes.create_string_buffer(library.cadencevad_state_size())
    assert library.cadencevad_init(state) == 0
    try:
        ...
        for hop_index, hop in enumerate(audio.split(config.feature.hop_samples)):
            ...
            result = library.cadencevad_extract_features(
                state,
        ...
        library.cadencevad_reset(state)
        ...
        for chunk_size in (17, 511, 3, 1_024, 99, 2_346):
            ...
            emitted = library.cadencevad_push(
                state,
        ...
        rejected = library.cadencevad_push(
            state,
